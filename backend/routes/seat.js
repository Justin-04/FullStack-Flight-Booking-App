...
router.put("/addSeat", (req, res) => {
  ...
  connection.query(updateSeatSql, [seatID], (err, seatResults) => {
    ...
    connection.query(checkSeatsSql, [flightID], (err, seatStatuses) => {
      ...
      const allSeatsBooked = seatStatuses.every(seat => seat.Status === "Booked");

      if (allSeatsBooked) {
        const updateFlightSql = "UPDATE flights SET flight_status = ? WHERE FlightID = ?";
        connection.query(updateFlightSql, [status_, flightID], (err) => {
          ...
          connection.query(deleteFromCart, [flightID], (err) => {
            ...
            res.send("Seat booked, flight marked 'Full', and removed from carts.");
          });
        });
      } else {
